...
make_manhattan <- function(in_kmers, plot_all, plot_inter){
...
if (dim(kmers)[1] > 0) {
  ...
  don$link <- paste("http://icci2.tau.ac.il:8000/cgi-bin/hgTracks?db=hub_112_Longissima&lastVirtModeType=default&lastVirtModeExtraState=&virtModeType=default&virtMode=0&nonVirtPosition=&position=",
  ...
  htmlwidgets::saveWidget(l, plot_all)

} else {

	system(paste("touch", plot_all))
	}
...
make_manhattan(snakemake@input[[1]], snakemake@output[[1]])
